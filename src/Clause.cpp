...
{
	//TODO beter lookup method (because list is sorted)
	assert(lit != NULL);
	assert(this->_size > 0);
	assert(this->clause != NULL);
	for(unsigned int i = 0; i < this->_size; i++)
...
bool Clause::operator==(const Clause & clause) const
{
	...
	for(unsigned int i = 0; i < this->_size; i++)
	{
		if(!this->Contains(clause.value[i]))
		{
			return false;
		}
	}
